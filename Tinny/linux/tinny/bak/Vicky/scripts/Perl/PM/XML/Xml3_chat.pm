...
sub new {
  ...
  if ($args) {
	...
	$indx = 1;
      ...
      #print    "\$O\-\>\{ATTR0\}\-\>\{\$keyyy\} =",    $O->{ATTR0}->{$keyyy}, "\n";
      #print "======================================\n";          
    }
    ...
    if( @_) {
       ...
       if( @_ ) { # there is attributes in the argument 
	     ...
	     print "In AutoLoad::Get \n";
             ...
             my $keyyy = $keyy.$id;
	     $args->{$key}=~ s/"|'//g;   #Added by chaitanya to clear problem of quotation marks in attribute values
             $O->{ATTR0}->{$keyyy} = $args->{$key};
          ...
          #print "----> temp     = ", $temp, "\n";
          #print "----> attr_key = ", $func.$id, "\n";

       }
       push(@{$O->{K}},$func);
       push(@{$O->{V}},escape($argm));
       push(@{$O->{KV}->{$func}},$argm);
       return;
    }
...
#      $str .= "<?xml version=\"1.0\"?>\n";
   ...
   $str .= "$space1<".$O->{name}.$O->{ATTR}->{$O->{name}.$id."0"}."/>"."\n";
    return $str;
    }
    else{
    ...
    foreach $key ( @ark ) {
	 ...
	 if (trim($temp) eq "") {
	   $str .= "$space1<".$key."$O->{ATTR}->{$key.$id.$j}/>"."\n";
	 }
	 else {
...
sub _parse {
    ...
    #convert empty tags to regular empty tags
    $str =~ s/<([-=a-zA-Z0-9:_.'"* ]*)\/>/<$1><\/$1>/g;

    #delete spaces inside the brackets 
    $str =~ s/<[ \n\t]+/</g;
    $str =~ s/[ \n\t]+>/>/g;
