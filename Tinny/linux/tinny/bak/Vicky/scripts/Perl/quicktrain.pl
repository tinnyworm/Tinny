...
$modelDir="$indir/001_gen_tryPara";
...
print "Step 1: Start the scaling...\n\n"; 
	...
	$scl_opts="-z scl,nozerocalc,binary -d std_kchk -R silent";
	$drv_filter="-f noposts";
	print "Running scaling $modName "; system ( "date" );
	$selectFile="select.mgen.$client.$month";
	!system ( "$FALCON4 -idir=$indir/data/$pct/$client.$month,select=$modelDir/select/$pct/mgen_$seg/$selectFile $scl_opts $drv_filter $modName.cnf >$modelDir/Log/log_part1_2.scl_$modName 2>$modelDir/Log/error_part1.scl_$modName" ) || die "Failed on scaling of step 2";
print "\n Done for step 1\n\n";
