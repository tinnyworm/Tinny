...
# 20071015 yxc replace the negative run-day by the original block date. 
...
while(<>){  # For each fraud record
  ...
  while ($auth){
    ...
    if($acc gt $acct_nr){
      ...
      $found = 1;
      $last_date = $date;
      if($result eq "A" or $result eq " "or $result eq "I"){
			$has_approved = 1;
  ...
  if($found){
    ...
    if ($last_date < $ff_date){ # write the orginal record out if the ffd after the last auths trandate#
			print substr($_,0,49) . "\n"; 
    }
    # Got approved transactions?
	 elsif($has_approved){
	  if ($last_apprv_date >= $ff_date)
	  {
	  ...
	  else 
	  {
       ...
       # No - write same fraud line but make type Z. Most likely old frauds.
		 # No any approved transactions from very begining of auths data. For
		 # scaling, we may just keep all frauds examplers and do nothing. 
     	 substr($_,19,1) = "Z";
