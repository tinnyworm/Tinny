...
proc print
  label
  data=random;
  var C001 C020 C021 C029 C038 C048 C051;
...
proc means
  ...
  var C038N;
  title 'A016. Statistics of Credit Line';
run;
...
proc print
  label
  data=C038f(obs=20);
  title 'A016. Credit Line (tran type = L)';
run;

proc chart
  data=nonmon;
  hbar C048 /type=percent missing;
  title 'A017B. Nonmon Numeric Type Only';
run;

proc chart
  data=nonmon;
  hbar type_num /type=percent missing;
  title 'A017A. Nonmon Type + Numeric Type';
run;

proc chart
  data=nonmon;
  hbar C051 /type=percent missing;
  title 'A018A. Nonmon Sub-Type';
run;

proc chart
  data=nonmon;
  hbar num_sub /type=percent missing;
  title 'A018B. Nonmon Numeric Type + Sub-Type';
run;
