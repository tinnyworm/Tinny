...
if ( $opt_m > 0 and $opt_m <= 100 ) { $minimum_pct = $opt_m } else { $minimum_pct = 0.1}

#Output final bins minimum size: 
if ( $opt_b > 0 and $opt_b <= 100 ) { $bin_min_pct = $opt_b } else { $bin_min_pct = $minimum_pct}
...
#------------------------------------------------------------------------------------------------------------
    ...
    if ($fld[0] =~ /Y|F|S/ || int($fld[0]) == "1") { $tag_not_sorted[$n]=1;}
    else { $tag_not_sorted[$n]=0;}
...
$num_counts = $n;
if ( $num_counts < 10 ) { die "Less that 10 input values. Process terminated\n";}
...
if ($counts_tot_Y-1 >0) { $factor_tot_Y = 1/($counts_tot_Y-1); } else { $factor_tot_Y =0; }
if ( $counts_tot_Y >0) {$var_avg_tot_Y = $var_sum_tot_Y/$counts_tot_Y;} else {$var_avg_tot_Y =0;}
    $var_std_tot_Y = $factor_tot_Y * ( $counts_tot_Y * $var_avg_tot_Y**2 - 2 * $var_avg_tot_Y * $var_sum_tot_Y + $var_sq_sum_tot_Y );
    if ($var_std_tot_Y<0){$var_std_tot_Y=0;}
    $var_std_tot_Y = sqrt($var_std_tot_Y);
if ($counts_tot_N-1 >0) { $factor_tot_N = 1/($counts_tot_N-1); } else { $factor_tot_N =0; }
if ( $counts_tot_N >0) {$var_avg_tot_N = $var_sum_tot_N/$counts_tot_N;} else {$var_avg_tot_N =0;}
    $var_std_tot_N = $factor_tot_N * ( $counts_tot_N * $var_avg_tot_N**2 - 2 * $var_avg_tot_N * $var_sum_tot_N + $var_sq_sum_tot_N );
    if ($var_std_tot_N<0){$var_std_tot_N=0;}
    $var_std_tot_N = sqrt($var_std_tot_N);

for ( $cut=1;$cut<=$num_cuts-1;$cut++ ) {
    if ($counts_Y{$thres[$cut]}-1 >0) { $factor_Y = 1/($counts_Y{$thres[$cut]}-1); } else { $factor_Y =0;}
    if ($counts_Y{$thres[$cut]} >0) {$var_avg_Y{$thres[$cut]} = $var_sum_Y{$thres[$cut]}/$counts_Y{$thres[$cut]}} else {$var_avg_Y{$thres[$cut]} = 0;}
    ...
    if ($var_std_Y{$thres[$cut]}<0){$var_std_Y{$thres[$cut]}=0;}
...
#    print "$thres[$cut] $counts_Y{$thres[$cut]} $var_sum_Y{$thres[$cut]} $var_sq_sum_Y{$thres[$cut]} $var_avg_Y{$thres[$cut]} $var_std_Y{$thres[$cut]}\n";
    if ($counts_N{$thres[$cut]}-1 >0) { $factor_N = 1/($counts_N{$thres[$cut]}-1); } else { $factor_N =0;}
    if ($counts_N{$thres[$cut]} >0) {$var_avg_N{$thres[$cut]} = $var_sum_N{$thres[$cut]}/$counts_N{$thres[$cut]}} else {$var_avg_N{$thres[$cut]} = 0;}
    ...
    if ($var_std_N{$thres[$cut]}<0){$var_std_N{$thres[$cut]}=0;}
...
if (length($var_avg_Y{$thres[$cut]})>9){$var_avg_Y{$thres[$cut]} = substr($var_avg_Y{$thres[$cut]},0,9);}
if (length($var_std_Y{$thres[$cut]})>9){$var_std_Y{$thres[$cut]} = substr($var_std_Y{$thres[$cut]},0,9);}
if (length($var_avg_tot_Y)>9){$var_avg_tot_Y = substr($var_avg_tot_Y,0,9);}
if (length($var_std_tot_Y)>9){$var_std_tot_Y = substr($var_std_tot_Y,0,9);}
if (length($var_avg_N{$thres[$cut]})>9){$var_avg_N{$thres[$cut]} = substr($var_avg_N{$thres[$cut]},0,9);}
if (length($var_std_N{$thres[$cut]})>9){$var_std_N{$thres[$cut]} = substr($var_std_N{$thres[$cut]},0,9);}
if (length($var_avg_tot_N)>9){$var_avg_tot_N = substr($var_avg_tot_N,0,9);}
if (length($var_std_tot_N)>9){$var_std_tot_N = substr($var_std_tot_N,0,9);}
}
