...
#
# $Source: /work/price/falcon/dvl/scripts/RCS/ufal_intersect.pl,v $
#
# 
#
...
if ( $opt_a ) {
	...
	if ( $dont_uncompress ) {
		system ( "$auths_file_str ufalcut -c1-19 | uniq > $auths_file_tmp" ); 
	} else {
		system ( "$auths_file_str ufalcut -c1-19 | uniq | gzip > $auths_file_tmp" ); 
	}
}
if ( $opt_c ) {
	if ( $opt_v ) { print "Creating $cards_file_tmp...\n"; }
	if ( $dont_uncompress ) {
		system ( "$cards_file_str ufalcut -c1-19 | uniq > $cards_file_tmp" ); 
	} else {
		system ( "$cards_file_str ufalcut -c1-19 | uniq | gzip > $cards_file_tmp" ); 
	}
}
if ( $opt_f ) {
	if ( $opt_v ) { print "Creating $frauds_file_tmp...\n"; }
	if ( $dont_uncompress ) {
		system ( "$frauds_file_str ufalcut -c1-19 | uniq > $frauds_file_tmp" ); 
	} else {
		system ( "$frauds_file_str ufalcut -c1-19 | uniq  | gzip > $frauds_file_tmp" ); 
	}
}
if ( $opt_n ) {
	if ( $opt_v ) { print "Creating $nonmons_file_tmp...\n"; }
	if ( $dont_uncompress ) {
		system ( "$nonmons_file_str ufalcut -c1-19 | uniq > $nonmons_file_tmp" ); 
	} else {
		system ( "$nonmons_file_str ufalcut -c1-19 | uniq | gzip > $nonmons_file_tmp" ); 
	}
}
if ( $opt_p ) {
	if ( $opt_v ) { print "Creating $paymnts_file_tmp...\n"; }
	if ( $dont_uncompress ) {
		system ( "$paymnts_file_str ufalcut -c1-19 | uniq > $paymnts_file_tmp" ); 
	} else {
		system ( "$paymnts_file_str ufalcut -c1-19 | uniq | gzip > $paymnts_file_tmp" ); 
	}
}
if ( $opt_t ) {
	if ( $opt_v ) { print "Creating $posts_file_tmp...\n"; }
	if ( $dont_uncompress ) {
		system ( "$posts_file_str ufalcut -c1-19 | uniq > $posts_file_tmp" ); 
	} else {
		system ( "$posts_file_str ufalcut -c1-19 | uniq | gzip > $posts_file_tmp" ); 
	}
}
...
if ( $opt_f && $opt_a ) {
	...
	$temp_str = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $auths_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_f && $opt_c ) {
	...
	$temp_str = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $cards_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_f && $opt_n ) {
	...
	$temp_str = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $nonmons_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_f && $opt_p ) {
	...
	$temp_str = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $paymnts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_f && $opt_t ) {
	...
	$temp_str = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# FRAUDS in AUTHS + POSTS

if ( $opt_f && $opt_a && $opt_t ) {
        print "--------------------------------------------------------------------------\n";
	if ( $opt_v ) { print "Computing the intersection of frauds in union of posts and auths  ...\n"; }

        if ( $dont_uncompress ) {
                system ( "col_merge $compress_flag -c1-19 $posts_file_tmp $auths_file_tmp | uniq > $temp_file" );  
        } else {
                system ( "col_merge                -c1-19 $posts_file_tmp $auths_file_tmp | uniq | gzip > $temp_file" );
        }

	$temp_str  = `fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $temp_file | wc -l`;  
	$temp_str =~ /(\d+)/;
	$posts_auths_frauds_intersect  = $1;
	if ($fraud_accounts != 0) { 
		$percent_tmp = ($posts_auths_frauds_intersect / $fraud_accounts) * 100;
		$~ = NUMFINPA;
		write;
	} else { print "Number of original frauds accounts ZERO!\n"; } 
...
if ( $opt_a && $opt_c ) {
	...
	$temp_str  = `fish.pl -b $auths_file_tmp -c 1-19 -s 1 $compress_flag $cards_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_a && $opt_n ) {
	...
	$temp_str  = `fish.pl -b $auths_file_tmp -c 1-19 -s 1 $compress_flag $nonmons_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_a && $opt_p ) {
	...
	$temp_str  = `fish.pl -b $auths_file_tmp -c 1-19 -s 1 $compress_flag $paymnts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_a && $opt_t ) {
	...
	$temp_str  = `fish.pl -b $auths_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# CARDS IN NONMONS
  if ( $opt_n ) {
	...
	$temp_str  = `fish.pl -b $cards_file_tmp -c 1-19 -s 1 $compress_flag $nonmons_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# CARDS IN PAYMENTS
  if ( $opt_p ) {
	...
	$temp_str  = `fish.pl -b $cards_file_tmp -c 1-19 -s 1 $compress_flag $paymnts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# CARDS IN POSTS
  if ( $opt_t ) {
	...
	$temp_str  = `fish.pl -b $cards_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# NONMONS IN PAYMENTS
  if ( $opt_p ) {
	...
	$temp_str = `fish.pl -b $nonmons_file_tmp -c 1-19 -s 1 $compress_flag $paymnts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# NONMONS IN POSTS
  if ( $opt_t ) {
	...
	$temp_str = `fish.pl -b $nonmons_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
# PAYMNTS IN POSTS
   if ( $opt_t ) 
   {
	...
	$temp_str = `fish.pl -b $paymnts_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_c && $opt_a && $opt_f ) {
	...
	if ( $opt_v ) { print "Computing the intersection of auths, cards, and frauds...\n"; }
        ...
        if ( $dont_uncompress ) {
                system ( "fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $auths_file_tmp > $temp_file" );  
        } else {
                system ( "fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $auths_file_tmp | gzip > $temp_file" );
        }

	$temp_str  = `fish.pl -b $temp_file -c 1-19 -s 1 $compress_flag $cards_file_tmp | wc -l`;  
	$temp_str =~ /(\d+)/;
...
if ( $opt_c && $opt_t && $opt_f ) {
	...
	if ( $opt_v ) { print "Computing the intersection of posts, cards, and frauds...\n"; }
        ...
        if ( $dont_uncompress ) {
                system ( "fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp > $temp_file" );  
        } else {
                system ( "fish.pl -b $frauds_file_tmp -c 1-19 -s 1 $compress_flag $posts_file_tmp | gzip > $temp_file" );
        }
...
###########################
# ESTIMATE ACTIVE ACCOUNTS
###########################

if ( $opt_a ) { 
	$active_accounts = int ( ( $auth_accounts - $frauds_auths_intersect ) / $sampling_rate );
	$percent_tmp = $sampling_rate * 100;
	$~ = NUMACTIVE;
	write;
}
if ( $opt_t ) { 
	$active_accounts = int ( ( $post_accounts - $frauds_posts_intersect ) / $sampling_rate );
	$percent_tmp = $sampling_rate * 100;
	$~ = NUMACTIVEPOST;
	write;
}
if ( $opt_c ) { 
	$total_accounts = int ( ( $card_accounts - $frauds_cards_intersect ) / $sampling_rate );
	$percent_tmp = $sampling_rate * 100;
	$~ = NUMTOTAL;
	write;
}
...
format NUMFINPA =
Number of frauds in posts U auths -> Total:@######### @###.#### %
                                             $posts_auths_frauds_intersect,$percent_tmp
.

format NUMACTIVE =
Est Number of Active Accounts (auths) at@#.## % Sampling Rate:@#########
                                       $percent_tmp,          $active_accounts
.

format NUMACTIVEPOST =
Est Number of Active Accounts (posts) at@#.## % Sampling Rate:@#########
                                       $percent_tmp,          $active_accounts
.

format NUMTOTAL =
Est Number of Total  Accounts         at@#.## % Sampling Rate:@#########
                                       $percent_tmp,          $total_accounts
.
