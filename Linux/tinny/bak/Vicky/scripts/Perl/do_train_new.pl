...
if (!(-d "$indir/data")) {
   ...
   foreach $file (@dataFile){
	    if (!(-e "$indir/data/$pct/$client.$month/$file")) {
...
$cmd_3="$FALCON4 -idir=$indir/data/$pct/$client.$month,select=$modelDir/select/$pct/mgen_$seg/$selectFile $scl_opts $drv_filter $modName.cnf >$modelDir/Log/log_part1_2.scl_$modName 2>$modelDir/Log/error_part1.scl_$modName";	
