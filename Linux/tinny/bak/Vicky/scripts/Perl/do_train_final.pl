...
$cmd_1="cp /home/yxc/models/Home2002.mod $modName.mod";
	...
	print "$FALCON4 $OPTALL $scl_opts $drv_filter $modName.cnf	>$modelDir/Log/log_part1_2.scl_$modName	2>$modelDir/Log/error_part1.scl_$modName\n" ;	
	unless ($DEBUG){
		 !system ( "$FALCON4 $OPTALL $scl_opts $drv_filter $modName.cnf >$modelDir/Log/log_part1_2.scl_$modName 2>$modelDir/Log/error_part1.scl_$modName" ) || die "Failed on scaling of step 2";
	}
print "\n Done for step 1\n\n";
